...
/**
 * Crypto Landing – Interactive Hero (Draft v1)
 * - Tailwind for layout & styling
 * - Framer Motion for UI motion
 * - R3F for subtle WebGL background
 *
 * Drop this component into a Next.js / React page to preview.
 */

function FloatingCoin({ idx = 0 }) {
  ...
  return (
    <mesh ref={mesh} castShadow receiveShadow>
      {/* TorusKnot feels techy & coin-like without textures */}
      <torusKnotGeometry args={[0.12 + Math.random() * 0.08, 0.035, 100, 8]} />
      <meshStandardMaterial metalness={0.85} roughness={0.15} color={idx % 2 ? "#00E5FF" : "#00F5A0"} />
    </mesh>
  );
}
...
function Scene() {
  ...
  useFrame((state) => {
    ...
    group.current.rotation.y = Math.sin(state.clock.getElapsedTime() * 0.08) * 0.15;
  });
...
export default function CryptoInteractiveHero() {
  ...
  const rx = useSpring(useTransform(y, [ -75, 75 ], [ 8, -8 ]), { stiffness: 140, damping: 12 });
  const ry = useSpring(useTransform(x, [ -75, 75 ], [ -8, 8 ]), { stiffness: 140, damping: 12 });
  ...
  return (
    <div className="relative min-h-[92vh] w-full overflow-hidden bg-[#05070B] text-white">
      ...
      <div className="pointer-events-none absolute inset-0 mix-blend-soft-light opacity-30" style={{ backgroundImage: "url('data:image/svg+xml;utf8,<?xml version=\"1.0\"?><svg xmlns=\"http://www.w3.org/2000/svg\" width=\"120\" height=\"120\" viewBox=\"0 0 120 120\"><filter id=\"n\"><feTurbulence type=\"fractalNoise\" baseFrequency=\"0.65\" numOctaves=\"2\" stitchTiles=\"stitch\"/></filter><rect width=\"100%\" height=\"100%\" filter=\"url(%23n)\" opacity=\"0.35\"/></svg>')" }} />
      ...
      <div className="relative z-10 mx-auto flex max-w-7xl flex-col items-center px-6 pt-24 md:pt-28">
        ...
        <div className="mt-10 grid w-full max-w-4xl grid-cols-2 gap-3 sm:grid-cols-4">
          {["온체인", "뉴스", "퀀트", "커뮤니티"].map((k, i) => (
            <div key={k} className="rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-center text-xs text-white/75">
              {k}
              <div className="mt-1 text-lg font-semibold text-white">{["12,457", "3.2ms", "+87%", "24/7"][i]}</div>
            </div>
          ))}
        </div>
      </div>
