  ...
  return (
    <form action={formAction} className="w-full max-w-sm space-y-4">
      ...
      <div className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 focus-within:ring-2 focus-within:ring-emerald-400/50">
        <label htmlFor="password" className="block text-xs text-white/60">비밀번호</label>
        <div className="mt-1 flex items-center">
          ...
          <button
            type="button"
            onClick={() => setShow(v => !v)}
            className="ml-2 rounded-md p-1 text-white/60 hover:text-white/90"
      ...
      <button
        type="submit"
        disabled={isPending}
        className="group inline-flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-emerald-500 to-cyan-500 px-4 py-3 font-semibold text-[#0b1220] shadow-[0_10px_30px_rgba(16,185,129,.35)] transition enabled:active:translate-y-[1px] disabled:opacity-60"
      >
        <LogIn className="h-5 w-5" />
        {isPending ? "확인 중…" : "로그인"}
      </button>

      <p className="text-center text-xs text-white/50">권한이 있는 사용자만 접근 가능합니다.</p>
    </form>
  );
}
